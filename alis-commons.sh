...
function sanitize_variable() {
	# remove disabled, remove unnecessary white spaces, trim leading and trailing
	local VARIABLE="$1"
	VARIABLE=$(echo "$VARIABLE" | sed -r 's/![^ ]*//g; s/ {2,}/ /g; s/^[[:space:]]*//; s/[[:space:]]*$//')
	echo "$VARIABLE"
}

function trim_variable() {
	local VARIABLE="$1"
	VARIABLE=$(echo "$VARIABLE" | awk '{$1=$1;print}')
	echo "$VARIABLE"
}
