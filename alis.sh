...
function check_variables() {
	...
	if [ "$DEVICE" == "auto" ]; then
		local DEVICE_BOOT=$(lsblk -oMOUNTPOINT,KNAME -P -M | grep 'MOUNTPOINT="/run/archiso/bootmnt"' | sed 's/.*KNAME="\(.*\)".*/\1/') #SC2155
		if [ -n "$DEVICE_BOOT" ]; then
			DEVICE_BOOT="/dev/$DEVICE_BOOT"
		fi
...
function install() {
	...
	sed -i '/Color/a ILoveCandy' "${MNT_DIR}"/etc/pacman.conf
	if [ "$PACMAN_PARALLEL_DOWNLOADS" == "true" ]; then
...
EOT
	...
	if [ "$PACKAGES_MULTILIB" == "true" ]; then
		sed -i '/#\[multilib\]/{s/^#//;n;s/^#//;}' "${MNT_DIR}"/etc/pacman.conf
	fi
}
...
EOT
	...
	if [ -n "$KEYAUTOREPEAT" ]; then
		OPTIONS="$OPTIONS"$'\n'"    Option \"AutoRepeat\" \"$KEYAUTOREPEAT\""
	fi
...
EOT
	...
	if [ -n "$SWAP_SIZE" ]; then
		echo -e "vm.swappiness=100\nvm.vfs_cache_pressure=100" >"${MNT_DIR}"/etc/sysctl.d/99-sysctl.conf
	fi
...
function bootloader() {
	...
	if [ "$VIRTUALBOX" != "true" ] && [ "$VMWARE" != "true" ] && [ "$KVM" != "true" ]; then
